...
final currentPage2Provider = StateProvider<int>((ref) => DateTime.now().month -1);
...
final scheduleCatchProvider = Provider.family<DateTime, DateTime>((ref, scheData) {
  int year = scheData.year;
  int month = scheData.month;
  int day = scheData.day;
  final data = ref.watch(scheduleDayProvider.notifier).setDateTime(year, month, day);
  return data;
});
...
final scheduleDayProvider = NotifierProvider<ScheduleDayNotifier, DateTime>(ScheduleDayNotifier.new);
...
// ref.watch(scheduleDayProvider).holidayJudge();
